...
function checkTableName($shortTName, $type=false)
{
	...
	if ("citas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("especialidades" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("medicos" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
 ...
 * @intellisense
 */
function GetTotals( $field, $value, $stype, $iNumberOfRows, $sFormat, $ptype, $pSet, $useRawValue = false )
{
	$days = 0;
	if($stype == "AVERAGE")
	{
...
function AfterFBLogIn( $pUsername, $pPassword, $pDisplayUsername, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("usuarios")
